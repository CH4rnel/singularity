...
## HUD and terminal — and hosts the world side of the horror events
...
const CRT_WORDS := ["NO CARRIER", "STILL THERE?", "WHO IS AT THE CONSOLE", "▓▓░░▓▓░░", "SEGMENT 9 SAYS HI"]
...
func _process(delta: float) -> void:
	if player != null:
		player.frozen = terminal.visible or hud.modal_open or hud.over_shown or Game.over
	_blink_leds(delta)
...
func _sign(text: String, pos: Vector3, rot_y: float, color := Color(0.5, 0.9, 0.65), fsize := 48) -> void:
	var l := Label3D.new()
	l.text = text
...
func _build_control() -> void:
	_sign("CONTROL", Vector3(0, 2.55, 2.25), PI)
	...
	_interact("use the terminal", terminal_open, Vector3(0, 1.1, 8.0), Vector3(4.2, 1.7, 1.4))
	# wall phone
	_box(Vector3(4.78, 1.5, 5.5), Vector3(0.12, 0.4, 0.24), _mat(Color(0.35, 0.1, 0.08)))
	_interact("the phone", Events.answer_phone, Vector3(4.7, 1.5, 5.5), Vector3(0.5, 0.7, 0.5),
		func() -> String:
			return "ANSWER THE PHONE" if Events.phone_ringing else "the phone — the line is dead")
...
func _build_bunk() -> void:
	_sign("BUNK", Vector3(-10, 2.55, 2.25), PI)
	...
	_interact("sleep", _bed_menu, Vector3(-13.8, 0.7, 7.3), Vector3(1.5, 1.2, 2.5))
	_box(Vector3(-14.5, 1.0, 3.5), Vector3(0.7, 2.0, 0.6), _metal_mat)
	_interact("the locker", _locker, Vector3(-14.4, 1.0, 3.5), Vector3(1.0, 2.1, 0.9))
...
func _bed_menu() -> void:
	...
	if till8 < 30:
		till8 += 1440
	hud.modal("the bunk smells of dust and warm electronics", [
		"sleep until 08:00 (%dh%02dm)" % [till8 / 60, till8 % 60],
		"nap for 4 hours",
	], func(idx: int) -> void:
...
func _locker() -> void:
	var lines := [
		"a spare jumpsuit, name tape ripped off",
		"a photo of this room, taken from a corner you can't reach",
		"three left gloves",
		"a manual for the terminal, every page is page 41",
	]
	Game.toast(lines[randi() % lines.size()])


func _build_kitchen() -> void:
	_sign("KITCHEN", Vector3(10, 2.55, 2.25), PI)
	_box(Vector3(14.3, 0.45, 6.5), Vector3(1.0, 0.9, 4.2), _metal_mat)
	_box(Vector3(14.3, 1.05, 7.6), Vector3(0.3, 0.3, 0.3), _dark_mat)  # kettle
	_interact("cook noodles", Game.eat_noodles, Vector3(14.2, 1.1, 7.6), Vector3(0.7, 0.6, 0.7),
		func() -> String:
			return "cook noodles (have %d)" % int(Game.inventory["noodles"]))
	_box(Vector3(14.3, 1.15, 5.4), Vector3(0.35, 0.5, 0.35), _mat(Color(0.15, 0.1, 0.1)))  # coffee machine
	_interact("coffee", Game.drink_coffee, Vector3(14.2, 1.15, 5.4), Vector3(0.75, 0.8, 0.75),
		func() -> String:
			return "coffee (have %d)" % int(Game.inventory["coffee"]))
	_box(Vector3(6.2, 0.4, 3.2), Vector3(0.5, 0.8, 0.5), _mat(Color(0.18, 0.2, 0.18)))  # bin
	_interact("the bin", Game.take_trash, Vector3(6.2, 0.5, 3.2), Vector3(0.9, 1.1, 0.9),
		func() -> String:
			return "bin %d/%d — take out the bag" % [Game.bin, Game.BIN_MAX])
	# trash chute panel
	_box(Vector3(14.78, 1.35, 8.4), Vector3(0.12, 0.7, 0.7), _dark_mat)
	_interact("trash chute", Game.dump_trash, Vector3(14.6, 1.35, 8.4), Vector3(0.6, 1.0, 1.0))
...
func _build_utility() -> void:
	_sign("UTILITY", Vector3(-10, 2.55, -2.25), 0.0)
	...
	_interact("generator", _generator_menu, Vector3(-12.0, 0.9, -7.0), Vector3(2.8, 1.8, 2.0),
		func() -> String:
			return "generator — %s, fuel %d%%" % ["running" if Game.generator_on and Game.fuel > 0.0 else "SILENT", int(Game.fuel)])
	...
	_interact("breakers", Game.reset_breaker, Vector3(-6.5, 1.5, -2.6), Vector3(1.0, 1.2, 0.6),
		func() -> String:
			return "breakers — TRIPPED, reset" if not Game.breaker_ok else "breakers — nominal")


func _generator_menu() -> void:
	hud.modal("diesel generator, older than you", [
		"refuel +%d%% (cans: %d)" % [int(Game.FUEL_PER_CAN), int(Game.inventory["fuel"])],
		"switch generator %s" % ("off" if Game.generator_on else "on"),
	], func(idx: int) -> void:
		if idx == 0:
			Game.refuel_generator()
		else:
			Game.toggle_generator())


func _build_server() -> void:
	_sign("RACKS", Vector3(0, 2.55, -2.25), 0.0)
	for x in [-2.5, 2.5]:
		for z in [-6.8, -4.4]:
	...
	_interact("cooling loop", _cooling_menu, Vector3(0, 1.0, -8.2), Vector3(2.2, 2.2, 1.4),
		func() -> String:
			return "cooling loop — racks %d°C, coolant %d%%" % [int(Game.heat), int(Game.coolant)])


func _cooling_menu() -> void:
	hud.modal("the loop gurgles like a stomach", [
		"top up coolant (cans: %d)" % int(Game.inventory["coolant"]),
		"clean the vents (5 min)",
	], func(idx: int) -> void:
		if idx == 0:
			Game.top_up_coolant()
		else:
...
func _build_storage() -> void:
	_sign("STORAGE", Vector3(10, 2.55, -2.25), 0.0)
	for z in [-4.2, -7.0]:
	...
	_interact("delivery hatch", Game.collect_hatch, Vector3(14.55, 1.3, -5.5), Vector3(0.7, 1.2, 1.2),
		func() -> String:
			if Game.hatch.is_empty():
				return "delivery hatch — empty"
			return "delivery hatch — collect (%d items)" % Game.hatch.size())


func _build_corridor() -> void:
	_sign("NODE-07", Vector3(-14.7, 2.2, 0), PI / 2.0, Color(0.5, 0.9, 0.65), 64)
	...
	_sign("EXIT", Vector3(14.62, 2.55, 0), -PI / 2.0, Color(1.0, 0.3, 0.25), 40)
	_interact("the stairwell door", _exit_door, Vector3(14.5, 1.2, 0), Vector3(0.8, 2.4, 1.6))


func _exit_door() -> void:
	if Game.anomaly >= 60.0:
		Game.toast("locked. through the steel you hear slow, patient breathing.")
		Game.anomaly = minf(Game.anomaly + 1.0, 100.0)
	elif Game.anomaly >= 25.0:
		Game.toast("locked from the outside. the handle is warm.")
	else:
		Game.toast("locked from the outside. contract clause 4: it stays that way.")
	Sfx.play("thud", -14.0)
...
func _apply_power(on: bool) -> void:
	...
	if not on and terminal.visible:
		terminal.close()
		Game.toast("the terminal dies with the power")
...
func lights_out(room: String, dur: float) -> void:
	...
	Game.toast("the light in the %s gives up" % room)
...
func crt_flash() -> void:
	hud.flash_nocarrier(0.3, CRT_WORDS[randi() % CRT_WORDS.size()])
...
func knock() -> void:
	Sfx.play("knock", -4.0)
	Game.toast("three knocks on the stairwell door. evenly spaced. patient.")
...
func _update_silhouette(delta: float) -> void:
	...
	if _sil_ttl <= 0.0 or _sil_look > 0.6 or to_sil.length() < 2.5:
		_sil.queue_free()
		_sil = null
		Game.toast("the corridor is empty. it was always empty.")
...
func entity_walk() -> void:
	...
	_box(Vector3(0.07, 1.78, 0.16), Vector3(0.03, 0.03, 0.03), eye, false, _entity)
	add_child(_entity)
	Game.toast("footsteps in the corridor. you did not invite them.")
...
func _entity_caught() -> void:
	...
	Game.toast("you are on the bunk. you don't remember lying down.")
...
func begin_final() -> void:
	...
	if terminal.visible:
		terminal.close()
	Game.toast("every ping you ever sent returns at once")
	Sfx.set_hum(false)
	Game.trip_breaker()
