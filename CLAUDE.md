...
- The five destinations (`pages/Wallet.vue` `TABS`/`TAB_OF`): wallet, feed, launchpad, DAO, Lain. Tokens, analytics, messages, accounts, network detail and security are places *inside* the wallet tab, reached from the portfolio — a sixth slot does not fit 390px. The desktop rail lists all eleven.
