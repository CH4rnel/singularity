...
The desktop window is the app's own frame, VSCode-style (`src/titlebar.html`, `src/frame.js`): one frameless `BaseWindow` holding two `WebContentsView`s — title bar above, the remote site below, which is why the bar cannot be part of the page. Windows and Linux draw the window buttons and have no application menu (attached to a frameless window it would be drawn over the bar), so the bar pops the same menus and `commandForInput` answers their accelerators by physical key; macOS keeps its traffic lights and native menu bar. `BaseWindow` has no web contents, so menu roles reaching for a focused `BrowserWindow` are dead — every page command is a `click` on the site's contents — and nothing has keyboard focus until the shell focuses the page. `--native-frame` gives the window back to the desktop.
...
- Five more places inside the wallet tab, all reached from the portfolio's second tile row and listed on the desktop rail (`WalletGasStation.vue`, `WalletProxy.vue`, `WalletEarn.vue`, `WalletBridge.vue`, `WalletBrowse.vue`). Each one is built on the same rule: it does what this build can honestly do, and names what it cannot.
...
- Inference API (`/api/ai/v1`, `config/ai.php`, `App\Services\Ai\*`, `App\Http\Controllers\Api\Ai\*`, `docs/ai-api.md`): OpenAI-compatible chat completions — including `stream: true` — in front of the provider keys already on this host (Groq via `GROQ_API_KEY`, OpenRouter via the key `LainChatService` uses). A gateway, not a proxy: the catalogue is an allowlist of Cyberia ids (`lain-fast`, `lain-large`, `lain-reason`, `lain-reason-mini`, `lain-free`) mapped to provider + upstream model, so an unknown id is a 400 rather than a bill, a model whose provider has no key leaves `/v1/models`, and a rate-limited or dead model falls back once to the model it names — reported as `served_by`, never after a stream's first byte. Access is a holding: `POST /api/ai/keys` issues a key to an address that signs a one-shot EIP-191 challenge (wording unlike login and the $LAIN room), the key is stored as a SHA-256 only, and `AiHolderGate` re-reads the address's share of the gate token on every request (default $LAIN ≥ 0.5%, vs 10% for the holders' room) — selling closes the API with no revocation, and an unreadable RPC is a 503 because the gate fails closed. Listing/revoking need the signature but not the holding; `ai:key issue … --service` mints gate-exempt keys for Cyberia's own daemons. Per-key quotas live in `AiUsageMeter` (minute in the rate limiter, day in the log so a cache flush cannot reset it), `max_tokens` is clamped not refused, and `ai_api_requests` stores metering with no column a prompt could go in (`ai:prune-usage`, 90 days). Errors use OpenAI's envelope; an upstream 401 becomes a 502, since the rejected credential is ours.
