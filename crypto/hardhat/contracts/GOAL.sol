...
/// @title Goal Bear Coin (GOAL) — Cyberia EVM token.
...
contract GOAL is ERC20, ERC20Burnable, ERC20Permit, Ownable {
    constructor(address initialOwner)
        ERC20("Goal Bear Coin", "GOAL")
        ERC20Permit("Goal Bear Coin")
        Ownable()
    {
    ...
    /// @notice Mint GOAL to `to`. Restricted to the owner.
    function mint(address to, uint256 amount) external onlyOwner {
        _mint(to, amount);
    }

    /// @notice Burn GOAL from `from`.
