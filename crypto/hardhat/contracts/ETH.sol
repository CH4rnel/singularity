...
/// @title ETH — bridged Ethereum representation on Cyberia EVM.
...
///             EVM supply backed 1:1 by the reserve on Ethereum.
/// @dev    Follows the standard Cyberia bridged-asset convention (OZ ERC20 +
///         Burnable + Permit + Ownable), same pattern as BTC/LTC/SOL/XMR. Ether
///         uses 18 decimals natively (wei), which matches the ERC20 default, so
///         no decimals() override is needed.
contract ETH is ERC20, ERC20Burnable, ERC20Permit, Ownable {
    ...
    /// @notice Mint ETH to `to`. Restricted to the bridge / relayer owner.
    function mint(address to, uint256 amount) external onlyOwner {
        _mint(to, amount);
    }

    /// @notice Burn ETH from `from` (used when bridging out).
