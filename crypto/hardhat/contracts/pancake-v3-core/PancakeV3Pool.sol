...
import '../pancake-v3-lm-pool/interfaces/IPancakeV3LmPool.sol';
...
contract PancakeV3Pool is IPancakeV3Pool {
    ...
    /// @dev Cyberia: upstream declares this `immutable`. Here it is storage so the factory owner can
    /// retune a live pool's swap fee without asking its liquidity to migrate to another tier -- the
    /// one operation a chain with little liquidity cannot afford. The value in the CREATE2 salt is
    /// unchanged and still identifies the pool, so pool addresses, `PoolAddress`, the path encoding
    /// and every SDK that derives an address stay exactly as they were.
    ...
    struct SwapCache {
        // the swap fee in effect for this swap, read once so the tick loop never touches storage
        uint24 fee;
    ...
    /// @dev Takes effect from the next swap only. Fees already accrued are untouched: they live in
    /// feeGrowthGlobal, which records what was actually charged rather than the rate that charged it.
