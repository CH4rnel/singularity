/**
 * Adds the BNB/CYBER LP farm pool (pid 24) to the MasterChef on Cyberia,
 * with allocPoint 100 — the same weight as every other active farm.
 *
 * Usage:
 *   npx hardhat run scripts/add-bnb-cyber-farm.ts --network cyberia
 */
...
const LP_TOKEN = "0xF4e6657946376Ed7f9c0aF26613C8863fFc97019" as const;
const WCYBER_ADDRESS = "0x78272aAd03E4b9d7A9134e874BA6d419B534F6c9" as const;
const BNB_ADDRESS = "0xF7655664D5D4b0681fdD4529438A1b667bCDc7E5" as const;
const RPC_URL = process.env.CYBERIA_RPC_URL || "https://rpc.cyberia.church";
const EXPECTED_PID = 24;
...
async function main() {
  console.log("=== Add BNB/CYBER LP farm pool ===");
  ...
  if (
    ...
    !pairTokens.has(BNB_ADDRESS.toLowerCase())
  ) {
    throw new Error(`LP token ${LP_TOKEN} is not the expected BNB/WCYBER pair`);
  }
  if (reserves[0] === 0n || reserves[1] === 0n) {
    throw new Error(`BNB/WCYBER pair ${LP_TOKEN} has no liquidity`);
  }
  ...
  console.log(`\n[add] BNB/CYBER LP (pid ${EXPECTED_PID})`);
