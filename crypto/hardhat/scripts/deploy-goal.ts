...
const artifact = JSON.parse(
  fs.readFileSync("./artifacts/contracts/GOAL.sol/GOAL.json", "utf8"),
);
...
// Optional override: GOAL_OWNER env var (defaults to deployer).
const owner = (process.env.GOAL_OWNER ??
  walletClient.account.address) as `0x${string}`;

console.log("Deploying Goal Bear Coin (GOAL)...");
...
console.log("GOAL deployed at:", receipt.contractAddress);
