...
const artifact = JSON.parse(
  fs.readFileSync("./artifacts/contracts/YTN.sol/YTN.json", "utf8"),
);
...
// Optional override: YTN_OWNER env var (defaults to deployer).
const owner = (process.env.YTN_OWNER ??
  walletClient.account.address) as `0x${string}`;

console.log("Deploying Yenten (YTN)...");
...
console.log("YTN deployed at:", receipt.contractAddress);
