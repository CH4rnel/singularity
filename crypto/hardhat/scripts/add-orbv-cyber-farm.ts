/**
 * Adds the ORBV/CYBER LP farm pool (pid 26) to the MasterChef on Cyberia,
 * with allocPoint 100 — the same weight as every other active farm.
 *
 * Usage:
 *   npx hardhat run scripts/add-orbv-cyber-farm.ts --network cyberia
 */
...
const LP_TOKEN = "0x2c3F33c0a91D5eaf7f01d52338A6eA72c95120D2" as const;
const WCYBER_ADDRESS = "0x78272aAd03E4b9d7A9134e874BA6d419B534F6c9" as const;
const ORBV_ADDRESS = "0x19E92D8475522FF6c8f3660372B9dc6674d85cC8" as const;
const RPC_URL = process.env.CYBERIA_RPC_URL || "https://rpc.cyberia.church";
const EXPECTED_PID = 26;
...
async function main() {
  console.log("=== Add ORBV/CYBER LP farm pool ===");
  ...
  if (
    ...
    !pairTokens.has(ORBV_ADDRESS.toLowerCase())
  ) {
    throw new Error(`LP token ${LP_TOKEN} is not the expected ORBV/WCYBER pair`);
  }
  if (reserves[0] === 0n || reserves[1] === 0n) {
    throw new Error(`ORBV/WCYBER pair ${LP_TOKEN} has no liquidity`);
  }
  ...
  console.log(`\n[add] ORBV/CYBER LP (pid ${EXPECTED_PID})`);
