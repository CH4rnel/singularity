/**
 * Deploys the Launchpad contract on Cyberia.
 *
 * Usage:
 *   npx hardhat run scripts/deploy-launchpad.ts --network cyberia
 */
...
const MIN_LIQUIDITY = 10_000n * 10n ** 18n; // 10k CYBER.sol
...
async function main() {
  ...
  console.log("minLiquidity:", MIN_LIQUIDITY.toString(), "(10k * 1e18)");
  ...
  const onchainCyber = await publicClient.readContract({
    address,
    abi: artifact.abi,
    functionName: "cyberSol",
  });
  ...
  console.log("\nOn-chain readback:");
  console.log("  cyberSol():    ", onchainCyber);
  ...
  const outPath = path.resolve("./deployments/cyberia-launchpad.json");
