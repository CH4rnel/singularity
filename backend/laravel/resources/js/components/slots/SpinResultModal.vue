    ...
    <div v-if="result" class="modal-backdrop" @click.self="emit('close')">
        <div class="modal">
            ...
            <div v-if="result.prize_payload.length > 0" class="prize">
                <div v-for="line in result.prize_payload" :key="line.mint" class="prize-line">
                    <span class="amount">+{{ formatAmount(line.amount, line.decimals) }}</span>
                    <span class="symbol">{{ line.symbol ?? symbolFor(line.mint) }}</span>
                </div>
            </div>
            <p v-if="result.burn_amount && BigInt(result.burn_amount) > 0n" class="burn">
                🔥 Сожжено: {{ formatAmount(result.burn_amount, 0) }} raw units
            </p>
            <p v-if="result.payout_tx_hash" class="tx">
                TX: <a :href="explorerUrl(result.payout_tx_hash)" target="_blank">{{ result.payout_tx_hash.slice(0, 12) }}…</a>
            </p>
            ...
            <div v-if="auditOpen" class="audit">
                <div><b>server_seed:</b> <code>{{ result.server_seed }}</code></div>
                <div><b>sha256:</b> <code>{{ result.server_seed_hash }}</code></div>
                <div><b>client_seed:</b> <code>{{ result.client_seed }}</code></div>
                <div><b>nonce:</b> <code>{{ result.nonce }}</code></div>
...
.prize-line { display: flex; gap: 8px; font-size: 1.4em; }
.amount { color: #4ade80; font-weight: bold; }
.burn { color: #f97316; }
.audit-toggle { background: none; border: 1px solid #444; color: #aaa; padding: 4px 12px; cursor: pointer; border-radius: 4px; margin-top: 16px; }
.audit { font-size: 0.8em; margin-top: 8px; word-break: break-all; }
.audit code { color: #93c5fd; }
.audit pre { background: #000; padding: 8px; border-radius: 4px; white-space: pre-wrap; }
.close { margin-top: 16px; padding: 8px 24px; background: #4ade80; color: #000; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; }
.tx a { color: #93c5fd; }
</style>
