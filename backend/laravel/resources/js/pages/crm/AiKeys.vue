...
<template>
    ...
    <div class="mk-grid" style="margin-top: 18px">
        ...
        <div
            ...
            style="padding: 15px"
            :style="
                summary.revoked > 0 ? { borderColor: 'rgba(255,77,77,.4)' } : {}
            "
        >
    ...
    >
        <span class="mk-t2" style="font-size: 12px">
            {{ t('keys.today') }}:
            <strong class="mk-num">{{ num(summary.used_today) }}</strong>
            {{ t('keys.instances') }} ·
            <strong class="mk-num">{{ num(summary.requests_today) }}</strong>
            {{ t('keys.requests') }} ·
            <strong class="mk-num">{{ num(summary.tokens_today) }}</strong>
            {{ t('keys.tokens') }}
        </span>
