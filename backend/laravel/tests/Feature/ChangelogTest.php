...
it('renders the changelog page', function () {
    $this->get(route('changelog'))
        ...
        ->assertInertia(fn (Assert $page) => $page
            ->component('Changelog')
            ->where('currentVersion', 'v0.9.1')
            ->has('releases', 10)
            ->where('releases.0.version', 'v0.9.1')
            ->where('releases.0.date', '2026-08-09')
            ->where('releases.0.title', 'Sealed mail between wallets')
            ->has('releases.0.sections', 1)
            ->where('releases.0.sections.0.label', 'Added')
            ->has('releases.0.sections.0.items', 5)
            ->where('release.current.version', 'v0.9.1')
