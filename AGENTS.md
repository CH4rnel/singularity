...
- The interface language is a **browser-side** choice and never Laravel's locale: `composables/useLocale.ts` holds one shared reactive `locale` for the whole app (English, Russian, Simplified Chinese), persists it in localStorage, guesses it from `navigator.language` on a first visit, and stamps a full BCP-47 tag onto `<html lang>` — Chinese needs its region there, because a browser given a bare `zh` can draw unified Han characters with Japanese glyph shapes. Copy lives in per-feature dictionaries (`lib/{wallet,download,progress,crm}Messages.ts`); **English is the only language a dictionary has to carry** and `t()` falls through to it, so a surface translated into two languages and one translated into three both work. A page's one-button switch cycles only the languages *its own* dictionary has — the operator-facing CRM is en/ru and therefore offers no Chinese — while the site-wide picker in `components/web3/WalletMenu.vue` lists all of them. Key sets and `{placeholders}` are pinned across every dictionary by `tests/Frontend/LocaleMessagesTest.mjs`, because a missing key is otherwise invisible: it renders in English and the screen keeps working.
...
- The desktop window is **the app's own frame**, VSCode-style (`src/titlebar.html`, `src/frame.js`, `src/preload-frame.js`): one frameless `BaseWindow` holding two `WebContentsView`s — the title bar across the top, the site underneath. The site is remote, so the bar cannot be part of the page; as a view of its own it also survives an OAuth redirect and the offline notice. Windows and Linux draw the window buttons in the bar and get no application menu at all (a menu attached to a frameless window is drawn over the bar) — the bar pops the same menus, and `commandForInput` answers the accelerators they print, matched on the physical key so Ctrl+R survives a Cyrillic layout; macOS keeps its traffic lights (`hiddenInset`) and its real menu bar. Two rules follow from `BaseWindow`: menu **roles** that reach for the focused `BrowserWindow` are dead here, so every item that touches the page is a `click` naming the site's contents through one command table; and no view has keyboard focus until it is given it, so the shell focuses the page on show and on window focus — otherwise the wallet opens with a password field that ignores typing. `--native-frame` / `CYBERIA_NATIVE_FRAME=1` hands the window back to the desktop.
